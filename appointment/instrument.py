...
def get_inst_info():  # 获取所有乐器的信息
    ...
    for inst in data:  # 统计数量，乐器信息的详情信息
        ...
        instinfo["typename"] = InstrumentType.objects.get(pk = instinfo["typepk"]).name # 获取该乐器对应的乐器类型的名称
...
def delete_inst(pk) : # 删除某一乐器
    ...
    if inst.order_set.filter( Q(status = Order.Status.UNPAID) | Q(status = Order.Status.PAID)).count() != 0:  # 检查是否有1.即将支付，或者2.已支付但未使用的订单存在
...
def delete_room(pk) : # 删除某一房间
    ...
    if room.order_set.filter( Q(status = Order.Status.UNPAID) | Q(status = Order.Status.PAID)).count() != 0:  # 检查是否有1.即将支付，或者2.已支付但未使用的订单存在
